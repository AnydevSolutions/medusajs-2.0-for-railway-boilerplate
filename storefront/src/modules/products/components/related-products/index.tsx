...
export default async function RelatedProducts({
  ...
  const setQueryParams = (): StoreGetProductsParams => {
    ...
    // Filtra por región si existe
    if (region?.id) {
      params.region_id = region.id
    }
  
    // Filtra por colección si existe
    if (product.collection_id) {
      params.collection_id = [product.collection_id]
    }
  
    // Excluir productos que sean giftcards
  ...
  return (
    <div className="product-page-constraint">
      ...
      <ul className="grid grid-cols-2 small:grid-cols-4 medium:grid-cols-4 gap-x-6 gap-y-8">
