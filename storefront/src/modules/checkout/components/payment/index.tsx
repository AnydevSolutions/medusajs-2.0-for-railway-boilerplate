    ...
    <div className="bg-white">
      <div className="flex flex-row items-center justify-between mb-6">
        <Heading
          level="h2"
          className={clx(
            "flex flex-row text-3xl-regular gap-x-2 items-baseline text-white px-5 py-1 border-[1px] rounded-2xl", // Clases comunes
            {
              "bg-black": isOpen, // Fondo negro cuando isOpen es true
              "bg-blue-500 opacity-50 pointer-events-none select-none": !isOpen && !paymentReady, // Fondo azul y deshabilitado si no está listo para pagar
              "bg-blue-500": !isOpen && paymentReady, // Fondo azul si está listo para pagar y isOpen es false
            }
          )}
        >
          Pago
          {!isOpen && paymentReady && <CheckCircleSolid className="text-white" />}
        </Heading>
        {!isOpen && paymentReady && (
          <Text>
            <button
